...
function Section({
  ...
  return (
    <section className={cn("panel p-4", className)}>
      <div className="mb-3 flex flex-wrap items-baseline gap-x-2.5 gap-y-1 border-b border-white/[0.06] pb-2">
        ...
        {meta && <span className="ml-auto shrink-0 font-mono text-[9px] text-text-tertiary">{meta}</span>}
      </div>
      {children}
    </section>
  );
...
export default function BriefOverlay() {
  ...
  return (
    ...
    >
      ...
      <div className="hud-scroll absolute inset-0 overflow-y-auto overflow-x-hidden">
            ...
            <div className="flex gap-1">
              {THREAT_LEVELS.map((lvl) => (
                <div
                  key={lvl}
                  className={cn(
                    "min-w-0 flex-1 overflow-hidden rounded-md py-1 text-center font-mono text-[7.5px] font-bold uppercase tracking-tight transition sm:text-[9px] sm:tracking-wider",
                ...
                {topAnoms.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="flex items-center gap-2 rounded-md bg-white/[0.03] px-2 py-1.5">
                      ...
                      <span className="shrink-0 font-mono text-[11px] font-semibold text-text-primary">{a.callsign}</span>
                      <span className="shrink-0 whitespace-nowrap font-mono text-[9px] uppercase tracking-wider" style={{ color: m.color }}>{a.category}</span>
                      <span className="min-w-0 flex-1 truncate pl-2 text-right font-mono text-[9px] text-text-tertiary">{a.reason}</span>
                    </div>
                  );
                })}
